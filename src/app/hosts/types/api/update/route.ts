...
export async function POST(req: Request) {
  try {
    ...
    const p = Schema.safeParse({ id: fd.get("id"), name: fd.get("name"), note: fd.get("note") });
    if (!p.success) return NextResponse.redirect(new URL(`${LIST}?toast=invalid&detail=ข้อมูลไม่ถูกต้อง`, req.url), { status: 303 });

    await prisma.hostType.update({ where: { id: p.data.id }, data: { name: p.data.name, note: p.data.note } });
    revalidatePath(LIST);
