...
export async function POST(req: Request) {
  try {
    ...
    if (!p.success) return NextResponse.redirect(new URL(`${LIST}?toast=invalid&detail=ไม่พบรหัส`, req.url), { status: 303 });

    await prisma.hostType.delete({ where: { id: p.data.id } });
    revalidatePath(LIST);
