...
export async function POST(req: Request) {
  try {
    ...
    const exists = await prisma.team.findUnique({ where: { name: p.data.name } });
    if (exists) {
      return NextResponse.redirect(new URL(`${LIST}/new?toast=exists&detail=มีทีมชื่อนี้แล้ว`, req.url), { status: 303 });
    }

    await prisma.team.create({ data: p.data });
    revalidatePath(LIST);
    return NextResponse.redirect(new URL(`${LIST}?toast=created&detail=สร้างทีมสำเร็จ`, req.url), { status: 303 });
  } catch (e: any) {
