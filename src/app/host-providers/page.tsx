...
import { deleteProvider } from "@/app/host-providers/actions";
...
export default async function ProvidersPage() {
  ...
  try {
    rows = await prisma.hostProvider.findMany({
      orderBy: { name: "asc" },
      select: { id: true, name: true, updatedAt: true },
    });
  } catch (e) {
    // If schema/migrate not ready, keep empty list to avoid crashing UI
    rows = [];
  }

  async function del(fd: FormData) {
    "use server";
    await deleteProvider({ ok: false, message: "" }, fd);
  }
