...
export async function POST(req: Request) {
  try {
    ...
    await prisma.domain.delete({ where: { id: p.data.id } });
    revalidatePath(LIST);
    return NextResponse.redirect(new URL(`${LIST}?toast=deleted&detail=ลบสำเร็จ`, req.url), { status: 303 });
  } catch (e: any) {
    const msg = e?.code === "P2025" ? "ไม่พบรายการ" : (e?.message || "ลบไม่สำเร็จ");
    return NextResponse.redirect(new URL(`${LIST}?toast=error&detail=${encodeURIComponent(msg)}`, req.url), { status: 303 });
  }
}